...
export const api = {
  ...
  captchaAufgabe: () => anfrage('/api/auth/captcha'),
  passwortVergessen: (kennung, captcha) => anfrage('/api/auth/passwort-vergessen', { methode: 'POST', daten: { kennung, captcha } }),
  passwortZuruecksetzen: (token, neuesPasswort) => anfrage('/api/auth/passwort-zuruecksetzen', { methode: 'POST', daten: { token, neuesPasswort } }),
  ...
  csvAnalyse: (text) => anfrage('/api/import/csv/analyse', { methode: 'POST', daten: { text } }),
  csvImport: (daten) => anfrage('/api/import/csv', { methode: 'POST', daten }),
  ...
  benachrichtigungLoeschen: (id) => anfrage(`/api/benachrichtigungen/${id}`, { methode: 'DELETE' }),
  freigabeSetzen: (daten) => anfrage('/api/konto/freigabe', { methode: 'POST', daten }),
  freigabeBeenden: () => anfrage('/api/konto/freigabe', { methode: 'DELETE' }),
  adminTestMail: (an) => anfrage('/api/admin/test-mail', { methode: 'POST', daten: { an } }),
  ...
  // Tauschbörse
  angebote: (filter = {}) => anfrage(`/api/boerse/angebote${abfrage(filter)}`),
  angebot: (id) => anfrage(`/api/boerse/angebote/${id}`),
  ...
  verkaufMelden: (id, daten) => anfrage(`/api/boerse/angebote/${id}/verkauf`, { methode: 'POST', daten }),
  kaufBestaetigen: (id, daten) => anfrage(`/api/boerse/verkaeufe/${id}/bestaetigen`, { methode: 'POST', daten }),
  ...
  blockieren: (id) => anfrage(`/api/boerse/anbieter/${id}/blockieren`, { methode: 'POST', daten: {} }),
  entblocken: (id) => anfrage(`/api/boerse/anbieter/${id}/blockieren`, { methode: 'DELETE' }),
  blockierte: () => anfrage('/api/boerse/blockiert'),
  unterhaltungen: () => anfrage('/api/boerse/nachrichten'),
  nachrichtenAnzahl: () => anfrage('/api/boerse/nachrichten/anzahl'),
  unterhaltung: (id) => anfrage(`/api/boerse/nachrichten/${id}`),
  nachrichtSenden: (id, text) => anfrage(`/api/boerse/nachrichten/${id}`, { methode: 'POST', daten: { text } }),
  bewerten: (id, daten) => anfrage(`/api/boerse/nachrichten/${id}/bewertung`, { methode: 'POST', daten }),
  haendler: () => anfrage('/api/boerse/haendler'),
  haendlerSpeichern: (daten) => anfrage('/api/boerse/haendler', { methode: 'PUT', daten }),
  ...
  haendlerTestStarten: () => anfrage('/api/boerse/haendler/test', { methode: 'POST', daten: {} }),
  adminTestzugang: (id, daten) => anfrage(`/api/admin/benutzer/${id}/testzugang`, { methode: 'POST', daten }),
  zahlungUebersicht: () => anfrage('/api/boerse/zahlung'),
  premium: () => anfrage('/api/premium'),
  premiumBuchen: (daten) => anfrage('/api/zahlung/checkout', { methode: 'POST', daten }),
  premiumKuendigen: (id) => anfrage(`/api/zahlung/abos/${id}/kuendigen`, { methode: 'POST', daten: {} }),
  schnaeppchenEinstellungen: () => anfrage('/api/schnaeppchen'),
  schnaeppchenSpeichern: (daten) => anfrage('/api/schnaeppchen', { methode: 'PUT', daten }),
  pushSchluessel: () => anfrage('/api/push/schluessel'),
  pushAbonnieren: (daten) => anfrage('/api/push', { methode: 'POST', daten }),
  pushAbbestellen: (endpoint) => anfrage('/api/push', { methode: 'DELETE', daten: { endpoint } }),
  pushTest: () => anfrage('/api/push/test', { methode: 'POST', daten: {} }),
  zahlungCheckout: (daten) => anfrage('/api/boerse/zahlung/checkout', { methode: 'POST', daten }),
  paypalBestaetigen: (subscriptionId) => anfrage('/api/boerse/zahlung/paypal/bestaetigen', { methode: 'POST', daten: { subscription_id: subscriptionId } }),
  aboKuendigen: (id) => anfrage(`/api/boerse/zahlung/abos/${id}/kuendigen`, { methode: 'POST', daten: {} }),
  zahlungPortal: () => anfrage('/api/boerse/zahlung/portal', { methode: 'POST', daten: {} }),
  adminZahlungen: () => anfrage('/api/admin/zahlungen'),
  adminZahlungErpNext: (id) => anfrage(`/api/admin/zahlungen/${id}/erpnext`, { methode: 'POST', daten: {} }),
  adminErstatten: (id, daten) => anfrage(`/api/admin/zahlungen/${id}/erstatten`, { methode: 'POST', daten }),
  adminGuthabenAuszahlen: (id) => anfrage(`/api/admin/benutzer/${id}/guthaben-auszahlen`, { methode: 'POST', daten: {} }),
  adminErpNextTest: () => anfrage('/api/admin/erpnext/test', { methode: 'POST', daten: {} }),
  ...
  adminMarktdaten: () => anfrage('/api/admin/marktdaten'),
  adminSicherungen: () => anfrage('/api/admin/sicherungen'),
  adminSicherungStarten: () => anfrage('/api/admin/sicherungen', { methode: 'POST', daten: {} }),
  adminSicherungExtern: () => anfrage('/api/admin/sicherungen/extern', { methode: 'POST', daten: {} }),
  adminSicherungExternTest: () => anfrage('/api/admin/sicherungen/extern/test', { methode: 'POST', daten: {} }),

...
export function erstelleMarktdatenDienst(db) {
  function uebersicht() {
    const g = db.prepare(`SELECT COUNT(*) AS anzeigen, SUM(beendet_am IS NULL) AS laufend, SUM(ergebnis = 'verkauft') AS verkauft,
        SUM(gewerblich) AS gewerblich, MIN(eingestellt_am) AS seit,
        SUM(verkauf_status = 'bestaetigt') AS bestaetigt, SUM(verkauf_status = 'gemeldet') AS gemeldet, COUNT(DISTINCT katalog_id) AS titel,
    ...
    // Meistverkaufte Titel – tatsächlicher Verkaufspreis, sonst letzter Angebotspreis; bestrittene Verkäufe zählen nicht
    const top = db.prepare(`SELECT m.katalog_id, k.titel, COUNT(*) AS verkauft, ROUND(AVG(COALESCE(m.verkaufspreis, m.preis_ende)), 2) AS preis_schnitt,
        MIN(COALESCE(m.verkaufspreis, m.preis_ende)) AS preis_min, MAX(COALESCE(m.verkaufspreis, m.preis_ende)) AS preis_max,
        SUM(m.verkauf_status = 'bestaetigt') AS bestaetigt
      FROM markt_angebote m LEFT JOIN katalog k ON k.id = m.katalog_id
      WHERE m.ergebnis = 'verkauft' AND COALESCE(m.verkauf_status, '') != 'bestritten' AND COALESCE(m.verkaufspreis, m.preis_ende) IS NOT NULL
    ...
    const verkaeufe = new Map(db.prepare(`SELECT substr(beendet_am, 1, 7) AS monat, COUNT(*) AS verkauft,
        ROUND(AVG(COALESCE(verkaufspreis, preis_ende)), 2) AS preis_schnitt
      FROM markt_angebote WHERE ergebnis = 'verkauft' AND COALESCE(verkauf_status, '') != 'bestritten' GROUP BY monat`).all().map((z) => [z.monat, z]));
    ...
    return {
      ...
      bestaetigt: g.bestaetigt ?? 0, gemeldet: g.gemeldet ?? 0,
  ...
  function angeboteCsv({ von, bis } = {}) {
    const zeilen = db.prepare(`SELECT m.id, m.katalog_id, k.titel, k.typ, p.name AS plattform, v.bezeichnung AS variante, m.art, m.zustand,
        ...
        m.aufrufe, m.anfragen, m.treffer, m.eingestellt_am, m.beendet_am, m.ergebnis, m.verkaufspreis, m.verkauf_status, m.ueber_zockdb
    ...
    return csv(['id', 'katalog_id', 'titel', 'typ', 'plattform', 'variante', 'art', 'zustand', 'vollstaendigkeit', 'region', 'anzahl',
      ...
      'eingestellt_am', 'beendet_am', 'ergebnis', 'verkaufspreis', 'verkauf_status', 'ueber_zockdb'], zeilen);
  }

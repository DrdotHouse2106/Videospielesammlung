...
export const EINSTELLUNGEN = [
  ...
  { schluessel: 'BACKUP_REMOTE', gruppe: 'Externe Sicherung', titel: 'Ziel', typ: 'auswahl', sicher: true,
    optionen: [['aus', 'Aus'], ['s3', 'S3-kompatibel (Hetzner, Backblaze, AWS, IONOS, MinIO …)'], ['webdav', 'WebDAV (Nextcloud, Storage Box, NAS …)'], ['sftp', 'SFTP (Storage Box, NAS, Server)']],
    hinweis: 'Täglich eine verschlüsselte Kopie der Datenbank (und neuer Uploads) zu einem Anbieter deiner Wahl.' },
  { schluessel: 'BACKUP_REMOTE_URL', gruppe: 'Externe Sicherung', titel: 'Adresse', typ: 'text', sicher: true,
    platzhalter: 'https://fsn1.your-objectstorage.com · https://u12345.your-storagebox.de · sftp://u12345.your-storagebox.de:23',
    hinweis: 'S3: Endpunkt ohne Bucket. WebDAV: Ordner-Adresse. SFTP: sftp://host:port/optionaler/ordner' },
  { schluessel: 'BACKUP_REMOTE_BUCKET', gruppe: 'Externe Sicherung', titel: 'S3: Bucket', typ: 'text' },
  { schluessel: 'BACKUP_REMOTE_REGION', gruppe: 'Externe Sicherung', titel: 'S3: Region', typ: 'text', platzhalter: 'us-east-1',
    hinweis: 'z. B. eu-central-1 (AWS), fsn1 (Hetzner), auto (Cloudflare R2). Im Zweifel us-east-1.' },
  { schluessel: 'BACKUP_REMOTE_USER', gruppe: 'Externe Sicherung', titel: 'Benutzer bzw. Access Key', typ: 'text', sicher: true },
  { schluessel: 'BACKUP_REMOTE_SECRET', gruppe: 'Externe Sicherung', titel: 'Passwort bzw. Secret Key', typ: 'geheim', sicher: true },
  { schluessel: 'BACKUP_REMOTE_PATH', gruppe: 'Externe Sicherung', titel: 'Unterordner', typ: 'text', platzhalter: 'zockdb' },
  { schluessel: 'BACKUP_REMOTE_PASSWORD', gruppe: 'Externe Sicherung', titel: 'Sicherungs-Passwort (Verschlüsselung)', typ: 'geheim', sicher: true,
    hinweis: 'Mindestens 12 Zeichen. Ohne dieses Passwort lassen sich die Sicherungen nicht wiederherstellen – unbedingt im Passwort-Manager aufbewahren!' },
  { schluessel: 'BACKUP_REMOTE_HOSTKEY', gruppe: 'Externe Sicherung', titel: 'SFTP: Host-Schlüssel (optional)', typ: 'text', sicher: true, platzhalter: 'SHA256:…',
    hinweis: 'Leer = der Schlüssel beim ersten Kontakt wird gemerkt und danach geprüft.' },
  { schluessel: 'BACKUP_REMOTE_DAYS', gruppe: 'Externe Sicherung', titel: 'Tägliche Stände aufbewahren (Tage)', typ: 'zahl', min: 1, max: 365 },
  { schluessel: 'BACKUP_REMOTE_MONTHS', gruppe: 'Externe Sicherung', titel: 'Monatliche Stände aufbewahren (Monate)', typ: 'zahl', min: 0, max: 1200,
    hinweis: '0 = unbegrenzt (Standard).' },
  { schluessel: 'BACKUP_REMOTE_UPLOADS', gruppe: 'Externe Sicherung', titel: 'Fotos und Scans mitsichern', ...JA_NEIN,
    hinweis: 'Jede Datei wird einmal übertragen (danach nur neue).' },
  ...
  { schluessel: 'PAYMENT_STRIPE_WEBHOOK_SECRET', gruppe: 'Zahlungen (Stripe, PayPal)', titel: 'Stripe: Webhook-Signaturgeheimnis', typ: 'geheim', sicher: true,
    hinweis: 'whsec_… – Webhook-Adresse: PUBLIC_URL + /api/zahlung/stripe/webhook, Ereignisse: checkout.session.completed, invoice.paid, charge.refunded, customer.subscription.updated, customer.subscription.deleted.' },
  ...
  { schluessel: 'PAYMENT_PAYPAL_WEBHOOK_ID', gruppe: 'Zahlungen (Stripe, PayPal)', titel: 'PayPal: Webhook-ID', typ: 'text',
    hinweis: 'Webhook-Adresse: PUBLIC_URL + /api/zahlung/paypal/webhook, Ereignisse: BILLING.SUBSCRIPTION.ACTIVATED, .CANCELLED, .EXPIRED, .SUSPENDED, PAYMENT.SALE.COMPLETED und PAYMENT.SALE.REFUNDED.' },
